...
const remove = () => {
    const lessoneBTN = document.querySelectorAll(".lesson-button")
    lessoneBTN.forEach((btn) => btn.classList.remove("active"))
}
...
const loadWordDitel = (id) =>{
const url = `https://openapi.programming-hero.com/api/word/${id}`
fetch(url)
.then((res) => res.json())
.then((data) =>  displayWordDitels(data.data))
}

const displayWordDitels = (word) =>{
   const ditelsBox = document.getElementById('ditels-container')
   ditelsBox.innerHTML = `
   
    <div class="space-y-5">
                    <div>
                        <h2 class="text-2xl font-bold">${word.word} (${word.pronunciation})</h2>
                    </div>
                    <div>
    ...
    words.forEach((word) => {
        ...
        <div class="bg-white rounded-xl shadow-sm text-center py-13 px-5 space-y-4">
            ...
            <div class="flex justify-between items-center">
                <button onclick="loadWordDitel(${word.id})" class="btn bg-[#1A91FF10] hover:bg-[#1A91FF40]"><i class="fa-solid fa-circle-info"></i> </button>
